...
function arraySortInfo(inputArray){
    if(inputArray.some(element=> typeof element !=='number')) return "Некоторые элементы не являются числами"
    
    if(inputArray.every((el,i)=> (i>0)? el>=inputArray[i-1]:true)) return "Массив отсортирован по возрастанию"

    if(inputArray.every((el,i)=> (i>0)? el<=inputArray[i-1]:true)) return "Массив отсортирован по убыванию"
